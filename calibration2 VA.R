...
for (p in min_period:max_period) {
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = data.frame(read_excel("DENC_catch_data_sim1.xlsx"))                                                                            
    ...
    #import and expand the wf_size_data so that each row represents a fish
    size_data_rd = subset(size_data, region==region1 & species=="reddrum", select= c(fitted_length, fitted_prob))
    size_data_rd$nfish = round(100000 * size_data_rd$fitted_prob, digits=0)
    sum(size_data_rd$nfish)
    
    row_inds <- seq_len(nrow(size_data_rd))
    size_data_rd <- size_data_rd[c(rep(row_inds, size_data_rd$nfish)), ]
    rownames(size_data_rd) = NULL
    size_data_rd = subset(size_data_rd, select=fitted_length)
    
    
    
    #draw random sample of sizes matching the number of fish caught
    random_sizes  = data.frame(size_data_rd[sample(nrow(size_data_rd), nrow(rd_catch_data)), ])
    colnames(random_sizes) = "fitted_length"
    random_sizes$fishid = 1:nrow(random_sizes)
    catch_size_data =  merge(random_sizes,rd_catch_data,by="fishid")
    ...
    minsize = rd_min
    maxsize = rd_max
    catch_size_data$keep_rd = ifelse(catch_size_data$fitted_length>=minsize & catch_size_data$fitted_length<=maxsize, 1,0) 
    catch_size_data$csum_keep_rd <- ave(catch_size_data$keep_rd, catch_size_data$tripid, FUN=cumsum)
    catch_size_data$keep_rd_adj = ifelse(catch_size_data$csum_keep_rd<=bag & catch_size_data$keep_rd==1, 1,0) 
    catch_size_data$release_rd = ifelse(catch_size_data$keep_rd_adj==1, 0,1) 
    
    
    catch_size_data= subset(catch_size_data, select=c(fishid, fitted_length, tot_catch_rd, tripid, keep_rd_adj, release_rd))
    names(catch_size_data)[names(catch_size_data) == "keep_rd_adj"] = "keep_rd"
    
    
    
    # generate sum of number of kept and released fish by tripid
    catch_size_data$tot_keep_rd=with(catch_size_data, ave(keep_rd, tripid, FUN = sum))
    catch_size_data$tot_rel_rd=with(catch_size_data, ave(release_rd, tripid, FUN = sum))
    
    
    # generate data set with total keep and release only
    catch_size_data1 = cbind(catch_size_data$tripid, catch_size_data$tot_keep_rd, catch_size_data$tot_rel_rd, catch_size_data$tot_catch_rd)
    colnames(catch_size_data1) = cbind("tripid", "tot_keep_rd", "tot_rel_rd", "tot_rd_cat")
    catch_size_data2 = as.data.frame(catch_size_data1[!duplicated(catch_size_data1), ])
    names(catch_size_data2)[names(catch_size_data2) == "tot_rd_cat"] = "tot_rd_catch"
    
    
    # append the zero wf catch trip_cost_data
    catch_size_data2 = bind_rows(catch_size_data2, rd_zero_catch)
    catch_size_data2 = subset(catch_size_data2, select=c(tripid, tot_keep_rd, tot_rel_rd, tot_rd_catch))
...
for (p in min_period:max_period) {
  ...
  for(d in 1:1) {
    ...
    param_draws_VANC = data.frame(read_excel("utility_params_draws_VA_NC.xlsx"))                                                                            
    param_draws_VANC1 = subset(param_draws_VANC, n==d)
    
    #Expected utility
    trip_data$vA = param_draws_VANC1$sqrt_sf_keep*sqrt(trip_data$tot_keep) +
      param_draws_VANC1$sqrt_sf_release*sqrt(trip_data$tot_rel) +  
      param_draws_VANC1$sqrt_bsb_keep*sqrt(trip_data$tot_keep_bsb) +
      param_draws_VANC1$sqrt_bsb_release*sqrt(trip_data$tot_rel_bsb) +  
      param_draws_VANC1$sqrt_wf_keep*sqrt(trip_data$tot_keep_wf) +
      param_draws_VANC1$sqrt_wf_release*sqrt(trip_data$tot_rel_wf) + 
      param_draws_VANC1$sqrt_rd_keep*sqrt(trip_data$tot_keep_rd) +
      param_draws_VANC1$sqrt_rd_release*sqrt(trip_data$tot_rel_rd) + 
      param_draws_VANC1$cost*trip_data$cost 
    ...
    mean_trip_data$vA_optout= param_draws_VANC1$optout*mean_trip_data$opt_out 
    mean_trip_data$vA_striper_blue= param_draws_VANC1$striper_blue*mean_trip_data$striper_blue 
...
pds_new_all_VA = as.data.frame(bind_rows(pds_new[[3]], pds_new[[4]], pds_new[[5]], pds_new[[6]]))
pds_new_all_VA[is.na(pds_new_all_VA)] = 0
pds_new_all_VA$state = state1
pds_new_all_VA$alt_regs = 0
pds_new_all_VA= subset(pds_new_all_VA, select=-c(Group.1, tot_sf_catch, tot_bsb_catch, tot_wf_catch, tot_rd_catch))
...
costs_new_all_VA = as.data.frame(bind_rows(costs_new_VA[[3]], costs_new_VA[[4]], costs_new_VA[[5]], costs_new_VA[[6]]))
