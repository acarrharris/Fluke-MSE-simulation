...
#GF check here
#Updated method to assign harvest and release of summer flounder, consistent with the calibration algorithm 
...
if(fluke_bag>0){
  ...
  # To do so, we parse out the full (adjusted) catch-at-length distribution into catch-at-length above the min. size limit (keep) 
  # and catch-at-length below the minimum size limit (release). We then adjust these probabilities into harvest-at-length 
...
if(bsb_bag>0){
  ...
  #GF check here
  ##Enter the prop_bsb_catch scalar here:
...
catch_size_data <- catch_size_data %>% 
  ...
  mutate(keep = ifelse(fitted_length>=fluke_min & fitted_length<=fluke_max,1,0)) %>% 
  group_by(period, catch_draw, tripid) %>% 
  # keep = case_when(
  # fitted_length>=minsize & fitted_length<=maxsize ~ 1,
  # TRUE ~ 0),
  mutate(csum_keep = cumsum(keep)) %>% 
  ungroup() %>% 
  mutate(
    keep_adj = ifelse(csum_keep<=fluke_bag & keep==1,1,0),
    # keep_adj = case_when(
...
catch_size_data= subset(catch_size_data, select=c(period, catch_draw, fishid, fitted_length, tot_sf_catch, tripid, keep_adj, release)) %>% 
...
if (state1 == "RI") {
  bsb_sc_data=subset(costs_new_all_RI, catch_draw<=nsamp, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>% 
    tibble()
...
if (state1 == "CT") {
  bsb_sc_data=subset(costs_new_all_CT, catch_draw<=nsamp, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>% 
    tibble()
...
if (state1 == "NY") {
  bsb_sc_data=subset(costs_new_all_NY, catch_draw<=nsamp, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>% 
    tibble()
...
if (state1 == "NJ") {
  bsb_sc_data=subset(costs_new_all_NJ, catch_draw<=nsamp, select=c(period, catch_draw,tripid,tot_keep_scup_base, tot_rel_scup_base, 
...
if (state1 == "DE") {
  bsb_sc_data=subset(costs_new_all_DE, catch_draw<=nsamp, select=c(period, catch_draw,tripid,tot_keep_wf_base, tot_rel_wf_base)) %>% 
...
if (state1 == "MD") {
  bsb_sc_data=subset(costs_new_all_MD, catch_draw<=nsamp, select=c(period, catch_draw,tripid,tot_keep_wf_base, tot_rel_wf_base)) %>% 
...
if (state1 == "VA") {
  bsb_sc_data=subset(costs_new_all_VA, catch_draw<=nsamp, select=c(period, catch_draw, tripid,
                                                                   tot_keep_wf_base, tot_rel_wf_base,
...
if (state1 == "NC") {
  bsb_sc_data=subset(costs_new_all_NC, catch_draw<=nsamp, select=c(period, catch_draw, tripid,
                                                                   tot_keep_wf_base, tot_rel_wf_base,
                                                                   tot_keep_rd_base, tot_rel_rd_base)) %>% 
    tibble()
  names(bsb_sc_data)[names(bsb_sc_data) == "tot_keep_wf_base"] = "tot_keep_wf"
