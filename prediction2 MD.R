...
for (p in min_period:max_period) {
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = subset(costs_new_all_MD, period ==p & catch_draw==i, select=c(tripid, tot_sf_catch))
    ...
    bsb_wf_data=subset(costs_new_all_MD, period ==p & catch_draw==i, select=c(tripid, tot_keep_bsb_base, tot_rel_bsb_base,
...
pds_new_all_MD = as.data.frame(bind_rows(pds_new[[1]], pds_new[[2]]))
