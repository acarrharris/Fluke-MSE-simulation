...
for (p in min_period:max_period) {
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = subset(costs_new_all_NY, period ==p & catch_draw==i, select=c(tripid, tot_sf_catch))
    ...
    bsb_sc_data=subset(costs_new_all_NY, period ==p & catch_draw==i, select=c(tripid,tot_keep_scup_base, tot_rel_scup_base, 
